...
class HPO(OnPolicyAlgorithm):
    ...
    def train(self) -> None:
        ...
        print("deltaY_num",self.deltaY_num)
            ...
            for epoch in range(self.n_epochs):
                ...
                for a in range(self.action_space.n):
                    ...
                    for i in range(self.batch_size):
                        ...
                        pltemp =  policy_loss_fn( x1[i].unsqueeze(0) , x2[i].unsqueeze(0) , (y[i]  ) .unsqueeze(0) )
                        od[ len(policy_losses2) ] = pltemp.item()
                        # p 
                        policy_losses2.append( abs_adv[i] * pltemp )
                ...
                deltaYnum = int(self.batch_size*self.action_space.n * self.rgamma )
                deltaYs = [0]*self.batch_size*self.action_space.n  
                deltaYcounter = 0
                # if  self.seed == 1234:
                #     pdb.set_trace()
                for key ,value in od:
                    ...
                    if deltaYcounter>= deltaYnum:
                        break
                    # deltaYs[key] = 1
                    deltaYs[key] = self.deltaY_num
                    deltaYcounter+=1
                for k in range( len(policy_losses2) ):
                    policy_losses2[k] = policy_losses2[k]*(1-2*deltaYs[k])
                ...
                #     not_0_loss_counter = 1
                policy_loss = th.stack(policy_losses2).sum() / len(policy_losses2)
