        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if self.classifier == "AM":
                    x1 = th.exp(log_prob - rollout_data.old_log_prob.detach()) # ratio
                    ...
                    x1 = th.sqrt(th.exp( log_prob - rollout_data.old_log_prob.detach())) # ratio
                    x2 = th.ones_like(x1.clone().detach())
                elif self.classifier == "AM-sub":
                    x1 = th.exp( log_prob )
                    x2 = th.exp(rollout_data.old_log_prob)
                elif self.classifier == "AM-square":
                    x1 = th.square(th.exp( log_prob - rollout_data.old_log_prob.detach())) # ratio
                    x2 = th.ones_like(x1.clone().detach())
                ...
                policy_loss_temp = abs_adv* policy_loss_fn(x1 , x2 , y )
                policy_loss = policy_loss_temp.mean()
                ...
                approx_kl_divs.append(th.mean(rollout_data.old_log_prob.detach() -  log_prob).detach().cpu().numpy())
