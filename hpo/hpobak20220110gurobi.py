        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                for i in range(self.batch_size):
                    ...
                    SVM.addConstr(ksi[i] >= self.alpha - y[i].item() *(1-2*deltaY[i])* ( x1[i].item() - x2[i].item() ) )
                    SVM.addConstr(ksi[i] <= self.alpha - y[i].item() *(1-2*deltaY[i])* ( x1[i].item() - x2[i].item() ) +M*(1-s[i ]))
