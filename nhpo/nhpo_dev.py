...
class NeuralHPO(OnPolicyAlgorithm):
    ...
    def train(self) -> None:
        ...
        # get log_prob for fixed log_prob
        replay_probs = []
        for a in range(self.action_space.n):
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                for i in range(self.batch_size):
                    ...
                    energy_loss += F.mse_loss(energy_function[i, uniform_action_idx[i]], new_temperature * (c_values[i].detach() + energy_function[i, uniform_action_idx[i]].detach() / self.temperature)) #8
