...
#####################################################################################################################
# class Env:
class miniBreakout(gym.Env):
    def __init__(self, ramping = None, random_state = None):
        super(miniBreakout, self).__init__()
        self.channels ={
            'paddle':0,
            'ball':1,
            'trail':2,
        ...
        self.action_map = ['n','f','u','r','l','d']
        
        # act_ite = np.zeros((len(self.action_map)), dtype=np.intc)
        self._action_set = (np.zeros((len(self.action_map)), dtype=np.intc))
        ...
        self.observation_space = spaces.Box(
            low=0, high=1, dtype=np.uint8, shape=(10,10,len(self.channels))
        )
