    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                gurobi_epsilon = self.alpha
                while True:
                    SVM = Model("robust_hpo")
                    flipped_proportion = 0.1
                    gamma = int(self.batch_size*flipped_proportion)
                    M = 1e10
                    # gurobi_epsilon = self.alpha
                    ksi = SVM.addVars(range(self.batch_size), vtype=GRB.CONTINUOUS)
                    deltaY = SVM.addVars(range(self.batch_size), vtype=GRB.CONTINUOUS)
                    s = SVM.addVars(range(self.batch_size), vtype=GRB.BINARY )
                    SVM.modelSense = GRB.MAXIMIZE
                    SVM.Params.outputFlag = 0
                    for i in range(self.batch_size):
                        SVM.addConstr(deltaY[i] >=0 )
                        SVM.addConstr(deltaY[i] <=1 )
                        SVM.addConstr(ksi[i] >=0 )
                        SVM.addConstr(ksi[i] >= gurobi_epsilon - y[i].item() *(1-2*deltaY[i])* ( x1[i].item() - x2[i].item() ) )
                        SVM.addConstr(ksi[i] <= gurobi_epsilon - y[i].item() *(1-2*deltaY[i])* ( x1[i].item() - x2[i].item() ) +M*(1-s[i ]))
                        SVM.addConstr(ksi[i] <= M*(s[i]) )
                    # SVM.setObjective( quicksum(ksi[i] for i in range(NUM_DATA) ) + gamma*q + quicksum(r[i] for i in range(NUM_DATA) ) )
                    SVM.addConstr(gamma == quicksum([ deltaY[i]  for i in range(self.batch_size) ]))
                    SVM.setObjective( quicksum(ksi[i] for i in range(self.batch_size) )   )
                    SVM.optimize()
                    if SVM.status == GRB.OPTIMAL or gurobi_epsilon>1:
                        break
                    else:
                        SVM.remove(SVM.getConstrs()[:])
                        gurobi_epsilon+=0.1
                # while SVM.status != GRB.OPTIMAL:
                    # print("SVM is not optimal")
                    # SVM.reset()
                # for robust_epoch in range(self.n_epochs):
                # for robust_epoch in range(10):
                #     robust_loss = th.tensor([0.], requires_grad=True).to(self.device)
                #     self.robust_delta_y.zero_grad()
                ...
                for i in range(self.batch_size):
                    ...
                    if self.aece == "WAE" or self.aece == "WCE":
                        # policy_loss += abs_adv[i] * policy_loss_fn( x1[i].unsqueeze(0) , x2[i].unsqueeze(0) , (y[i]*(1-2*deltaY[i].clone().detach() ) ) .unsqueeze(0) )
                        policy_loss += abs_adv[i] * policy_loss_fn( x1[i].unsqueeze(0) , x2[i].unsqueeze(0) , (y[i]*(1-2*deltaY[i].x ) ) .unsqueeze(0) )
                    else:
                        # policy_loss += policy_loss_fn( x1[i].unsqueeze(0) , x2[i].unsqueeze(0) , (y[i]*(1-2*deltaY[i].clone().detach() ) ) .unsqueeze(0) )
                        policy_loss += policy_loss_fn( x1[i].unsqueeze(0) , x2[i].unsqueeze(0) , (y[i]*(1-2*deltaY[i].x ) ) .unsqueeze(0) )
